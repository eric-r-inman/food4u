...
emit "select 'insert into storage_items (id, location_id, name, needs, count, position) values ('||quote(id)||', '||quote(location_id)||', '||quote(name)||', '||(case when needs then 'true' else 'false' end)||', '||count||', '||position||');' from storage_items where location_id not in (select id from storage_locations where $minted) order by location_id, position;"
...
emit "select 'insert into recipe_ingredients (id, recipe_id, name, needs, count, position) values ('||quote(id)||', '||quote(recipe_id)||', '||quote(name)||', '||(case when needs then 'true' else 'false' end)||', '||count||', '||position||');' from recipe_ingredients order by recipe_id, position;"
